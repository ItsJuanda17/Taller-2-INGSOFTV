...
tasks.withType<Test> {
    ...
    // Inside the Jenkins container, the test JVM cannot reach the published
    // port via "localhost" — that's the Jenkins container, not the host.
    // host.docker.internal is the alias Docker Desktop wires into every
    // container. Outside containers (Windows local), the default "localhost"
    // already routes correctly, so we only force the override under Jenkins.
